...
*Cmdlets* are native PowerShell commands, not stand-alone executables. Cmdlets are collected into PowerShell modules that can be loaded on demand. Cmdlets can be written in any compiled . NET language or in the PowerShell scripting language itself. (Cmdlets are like built-in "tools" that PowerShell knows how to use, not separate applications. - Each cmdlet performs a specific, focused task — like getting data, modifying it, or sending it elsewhere.)
...
(A module is a package that groups together related PowerShell functionality — such as cmdlets, functions, variables, and aliases — so you can easily share or reuse them.)
...
A .ps1 file contains a PowerShell script, which is a sequence of commands (possibly calling many cmdlets).
A cmdlet is a single command, implemented in C# or PowerShell, usually distributed in a module, not a .ps1 file.
...
When you use Write-Output, whatever you send is available for:
- Further processing by another cmdlet
- Or display in the console (if it’s the final output)

`Write-Host` - Writes text directly to the console (host) — not to the pipeline.
It’s purely for display. Anything you write with Write-Host:
- Can’t be captured
- Can’t be piped
- Disappears from the data stream
